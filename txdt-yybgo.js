/*
------------------------------------------
@Author: sm (adapted for YYB Go)
@Date: 2026.07.17
@Description: 腾讯地图 签到领现金/现金余额查询
cron: 18 8 * * *
------------------------------------------
变量名：txdt
变量值：YYB Go 里的 openid，多账号用 & 或换行

依赖变量：
yyb_go_url   YYB Go 服务地址，如 http://192.168.1.x:8000
------------------------------------------
*/
...
const YYB_GO_URL = (process.env.yyb_go_url || "http://127.0.0.1:8000").replace(/\/$/, "");
...
// 改为调 YYB Go 的 /wxapp/getCode 接口
async function getWxCode(openid) {
    const { status, data } = await request({
        method: "POST",
        url: `${YYB_GO_URL}/wxapp/getCode`,
        headers: { "content-type": "application/json" },
        data: { ref: openid, app_id: APP.appid },
    });
    // YYB Go 返回: { code:0, data:{ openid:"xxx", result:{ code:"xxx", errMsg:"login:ok" } } }
...
class TencentMap {
    ...
    async miniLogin() {
        if (!this.account.openid) throw new Error("账号格式错误，请配置 YYB Go 里的 openid");
